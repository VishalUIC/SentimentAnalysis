...
def gen_BagOfWords(recds, allAttribs, header='text'):
    data = []
    for rec in recds:
        if header not in rec:
            print(header)
        if len(rec.get(header)) > 0:
...
def genVocab(dictFile, header='text'):
    allFtrs = []
    for row in dictFile:
...
def dataClean(flNm="../data/training-Obama-Romney-tweets.xlsx"):
    ...
    df_obama['text'] = df_obama['text'].apply(cleanFnc)
    df_romney['text'] = df_romney['text'].apply(cleanFnc)
    ...
    for tweet in obama_rec:
        tweet['text'] = list(map(remSymFnc, tweet['text'].split()))
        tweet['text'] = [ele.lower() for ele in tweet['text'] if not 'http' in ele and not ele.isdigit()]
    ...
    for tweet in obama_rec:
        tweet['text'] = list(filter(lambda x: x not in stopWords, tweet['text']))
    ...
    p = PorterStemmer()
    for tweet in obama_rec:
        tweet['text'] = list(map(lambda x: p.stem(x), tweet['text']))
...
def feature_gen(flNm):
    ...
    for c in range(len(candid_rec)):
        i = candid_rec[c]
        dict[c] = [i['aspect_term']]
        i['text'] = re.findall(r"[\w']+", "".join(i['text'].lower().split("[comma]"))) + [i['aspect_term']]

    candid_rec = candid_rec[:900]
    candid_rec = clean(candid_rec)
...
def getClf(methodName):
    if methodName == 'logistic':
        ...
        return OneVsRestClassifier(
            BaggingClassifier(svm.SVC(kernel='linear'), max_samples=1.0 / n_estimators, n_estimators=n_estimators,
                              n_jobs=10))
...
def __main__():
    ...
    parser.add_argument('-cv', '--cross_validate', help='Use this for classifier training and cross validation',
                        required=False)
...
def clean(rec):
    stopWords = set(stopwords.words("english"))
    for tweet in rec:
        tweet['text'] = list(filter(lambda x: str(x) not in stopWords, tweet['text']))

    p = PorterStemmer()
    for tweet in rec:
        tweet['text'] = list(map(lambda x: p.stem(str(x)), tweet['text']))
...
flNm = "../data/obamaTweets_test.json"
flNm = "dataTrain.json"
json_data = open(flNm).read()

jsonDict = json.loads(json_data)

for sent in jsonDict:
    # sent['aspect_sentence'] =
    print(sent)
    pass
...
X, y, _ = feature_gen(flNm)
print("Starting feature generation steps __main__")
train_x, train_y, all_attribs = feature_gen(flNm)
print("Feature generation complete __main__\n")

print("Classifier training started __main__")
clf_obj = train('bayes', train_x, train_y)
print("Classifier training complete __main__\n")

print("Testing on data started __main__")
test(clf_obj, all_attribs, flNm)
print("Testing on data completed __main__")

"""
flNm = "../data/dataTrain.json"
